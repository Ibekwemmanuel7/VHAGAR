...
def render_evidence_pack(result, events=None, *, title="Wildfire Event Evidence Pack",
    ...
    table = (f"<table><thead><tr><th>Location</th><th>Status</th><th>Distance (m)</th>"
             f"<th>Event</th><th>Last seen (UTC)</th><th>Age (h)</th><th>Sensors</th>"
             f"<th>Confidence</th></tr></thead><tbody>{_rows(affected)}</tbody></table>"
...
{table}
<div class="disclosure"><b>Disclosure.</b> {_esc(disclosure)}</div>
