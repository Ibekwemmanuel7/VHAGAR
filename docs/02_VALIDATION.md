...
## Rule: SYNTHETIC labels on rendered and published assets

No synthetic metric may appear in any rendered or published asset (video,
image, slide, social post) without the word SYNTHETIC in the same visual
element as the number. This applies to the `fire_tracker/` renderers and to any
figure exported for outreach. A synthetic benchmark placed next to real-fire
footage without that label is treated as an integrity defect, not a cosmetic
one. The `t4-nextday` AP figures (corrector 0.871, physics 0.855, persistence
0.466) are the circular synthetic demo (truth generated by the physics prior's
own family) and must always carry the SYNTHETIC label and a
"real-data validation pending" note until the real-WildfireSpreadTS run in
Phase 2 replaces them. Per docs/15, any next-day AP much above 0.5 is almost
certainly a leaky split and may not be presented as a real-data result.
